window.addEventListener('load', function () {
  ...
  // 各要素取得---------------------------------------------------
    ...
    const regexp = /^[0]+[0-9]/;
    const regexp1 = /[0-9]/;
    const regexp2 = /[1-9]|1[0-2]{2}/;
    // const regexp2_1 = /[1-9]{1}/;
    // const regexp2_2 = /1[0-2]{2}/;
    const regexp3 = /[1-9]|[12][0-9]|3[01]/;
  ...
  // 不正入力時のバリデーション---------------------------------------------------
    ...
    month_box.addEventListener('blur', function(){
      const month = document.getElementById("month").value;
      const regexp2 = /[1-9]|1[0-2]/;
      if(month !== ''){
        if(!month.match(regexp2)|| month.length > 2){
          errorElement1.innerText = "値が不正です。";
        };
    ...
    day_box.addEventListener('blur', function(){
      ...
      const regexp3 = /[1-9]|[12][0-9]|3[01]/;
      if(day !== ''){
        if(!day.match(regexp3) || day.length > 2){
          errorElement1.innerText = "値が不正です。";
        }else if(day.match(regexp3)){
          errorElement1.innerText = "";
        };
      };
    });
