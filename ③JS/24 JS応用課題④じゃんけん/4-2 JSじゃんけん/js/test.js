...
function onClickPlayerIcon(self){
  var hands = document.getElementsByClassName("choice");
  choiceAry = Array.prototype.slice.call(hands);

  // すでにselectのクラス値が与えられているHTMLエレメントのselectedを消去する。
  document.getElementsByClassName.forEach(function(){
    document.getElementsByClassName(this).toggleClass("selected");
  });
  // クリックされたHTMLエレメントにselectedのクラス値を与える
  document.getElementsByClassName(self).toggleClass("selected");
